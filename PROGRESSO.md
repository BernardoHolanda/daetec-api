...
- ✅ **Usuário: email + senha forte** (lição 29): `email` **obrigatório/único**
  ...
  `admin` / `Senhaforte1`.
- ✅ **Auditoria da venda** (lição 30): `Venda.registrado_por_id` (FK not-null p/
  `usuarios`) grava **quem** registrou. `POST /vendas` passou a **exigir login** →
  fecha o furo de rota aberta. (`fechada_por`/auditoria de conta ficou de fora por
  decisão.)
- ✅ **CORS** (`CORSMiddleware`): libera o front (origens do `.env` `CORS_ORIGINS`,
  default `localhost:5173`) a chamar a API. Coisa de **navegador** — `curl` ignora.
...
| 30 | Auditoria (quem registrou) | `Venda.registrado_por_id` (FK **not-null** p/ `usuarios`) grava **quem** criou a venda. Pra saber quem, `POST /vendas` passou a **exigir login** (`Depends(get_current_user)`) → fecha o furo de rota aberta. **Threading**: o router pega o `Usuario` logado e passa `usuario.id` pro crud (`criar_venda(..., registrado_por_id)`) — router sabe de HTTP/auth, crud só quer o id. Migration: autogenerate criou a FK **sem nome** de novo → nomeada à mão (`vendas_registrado_por_id_fkey`). |
